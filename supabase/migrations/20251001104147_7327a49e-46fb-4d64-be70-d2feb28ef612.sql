-- DROP funções existentes que vamos recriar
DROP FUNCTION IF EXISTS msg_get_user_threads(uuid);
DROP FUNCTION IF EXISTS msg_get_thread_messages(uuid, uuid, int, int);
DROP FUNCTION IF EXISTS msg_mark_messages_read(uuid, uuid);

-- Adicionar coluna read_at na tabela chat_receipts
ALTER TABLE chat_receipts ADD COLUMN IF NOT EXISTS read_at timestamptz;
ALTER TABLE chat_receipts ADD COLUMN IF NOT EXISTS delivered_at timestamptz DEFAULT NOW();
...
BEGIN
    ...
    SELECT 
        ...
        -- Mensagens não lidas (otimizado com receipts)
        COALESCE((
            SELECT COUNT(*)
            FROM chat_messages cm
...
AS $$
BEGIN
    -- Inserir receipts para todas mensagens não lidas
...
CREATE INDEX IF NOT EXISTS idx_chat_receipts_user_message 
    ON chat_receipts(user_id, message_id) WHERE read_at IS NULL;
