...
-- 4. Create view for public broker data (business info only)
CREATE OR REPLACE VIEW public.public_broker_business AS
SELECT 
  id,
  name,
  username,
...
GRANT SELECT ON public.public_broker_business TO anon, authenticated;

-- 5. Add function to check if user is admin (security definer)
