...
-- ===== MINISITE CONFIGS (perfil → minisite)
ALTER TABLE public.minisite_configs ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='minisite_configs' AND policyname='minisite_configs_owner_rw') THEN
    CREATE POLICY minisites_cfg_owner_rw
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='indications' AND policyname='indications_user_access') THEN
    CREATE POLICY indications_user_access
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='indication_discounts' AND policyname='indication_discounts_visible_by_owner') THEN
    CREATE POLICY indication_discounts_visible_by_owner
...
-- ===== IMÓVEIS (para não quebrar "Meus Imóveis / Marketplace" quando vazios)
ALTER TABLE public.imoveis ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='imoveis' AND policyname='imoveis_public_read') THEN
    CREATE POLICY imoveis_public_read ON public.imoveis FOR SELECT
    USING (is_public = true AND visibility = 'public_site');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='imoveis' AND policyname='imoveis_owner_rw') THEN
    CREATE POLICY imoveis_owner_rw ON public.imoveis FOR ALL
