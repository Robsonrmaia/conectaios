-- Chat messaging system complete implementation - Fixed
...
-- 1.7 Useful Indexes (using correct column name 'read')
...
DROP POLICY IF EXISTS chat_receipts_all ON public.chat_receipts;
...
DROP POLICY IF EXISTS chat_presence_self ON public.chat_presence;
CREATE POLICY chat_presence_self ON public.chat_presence
FOR ALL TO authenticated
USING (user_id = auth.uid())
WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS chat_presence_read_others ON public.chat_presence;
