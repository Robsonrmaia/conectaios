-- Phase 1: Critical Security Fixes (Handle Existing Policies)
...
DROP POLICY IF EXISTS "Public can view basic business info only" ON public.conectaios_brokers;
DROP POLICY IF EXISTS "Authenticated users can view extended business info" ON public.conectaios_brokers;
DROP POLICY IF EXISTS "Brokers can view their own complete profile" ON public.conectaios_brokers;
DROP POLICY IF EXISTS "Brokers can update their own profile" ON public.conectaios_brokers;
DROP POLICY IF EXISTS "Users can create their broker profile" ON public.conectaios_brokers;
DROP POLICY IF EXISTS "Admins can view all broker profiles" ON public.conectaios_brokers;
DROP POLICY IF EXISTS "Service role can insert brokers" ON public.conectaios_brokers;

-- Create secure policies for conectaios_brokers
