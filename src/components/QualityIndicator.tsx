...
import { PropertyQualityModal } from "./PropertyQualityModal";
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className={`flex items-center gap-1 ${className}`}>
              <Badge 
                variant="secondary"
                className={`${quality.color} text-white ${getSizeClasses()} flex items-center gap-1 cursor-pointer hover:opacity-80 transition-opacity`}
                onClick={() => property && setShowModal(true)}
              >
                {getIcon()}
                {score}%
              </Badge>
